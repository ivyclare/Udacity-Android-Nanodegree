...
public class FavoriteProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case MOVIE: {
                long id = sqLiteDatabase.insert(FavoriteContract.Tables.MOVIE, null, values);
                if (id == -1) {
                    sqLiteDatabase.close();
                    throw new SQLException("Failed to insert row into " + uri);
                }
                Uri returnUri = FavoriteContract.MovieEntry.buildMovieUri(id);
                notifyChange(uri);
                /*
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case MOVIE: {
                rowsDeleted = sqLiteDatabase.delete(FavoriteContract.Tables.MOVIE, selection, selectionArgs);
                break;
            }
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        uriMatcher.addURI(FavoriteContract.CONTENT_AUTHORITY, FavoriteContract.PATH_MOVIE, MOVIE);

        //Match uri with movie id
        uriMatcher.addURI(FavoriteContract.CONTENT_AUTHORITY, FavoriteContract.PATH_MOVIE + "/#", MOVIE_WITH_ID);
